...
const scene = {
  activeCamera: {},
  createPickingRay: () => ({ origin: { x: 0, y: 0, z: 0 }, direction: { x: 0, y: 0, z: 1 } }),
}
...
const setup = () => {
  ...
  const pointer = new FlatPointer(canvas as unknown as HTMLCanvasElement, scene as never)
  return { canvas, pointer, cleanup: () => { pointer.dispose(); canvas.remove() } }
}
...
describe('FlatPointer: the camera keeps its own gestures', () => {
  ...
  it('ignores a right-drag', () => {
    const { canvas, pointer, cleanup } = setup()
    canvas.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, pointerId: 1, button: 2 }))
    expect(pointer.active).toBe(false)
    cleanup()
  })
})

...
const node = (name: string, parent?: object) => ({ name, parent: parent ?? null })
  ...
  ({
    pieces: new Map(
      Object.entries(pieces).map(([id, body]) => [
        id,
        { piece: { id, at: [0, 0, 0] }, at: [0, 0, 0], scale: 1, handles: new Map(), ...body },
      ])
    ),
...
describe('selection', () => {
  it('indexes element bodies by their mesh and node bodies by themselves', () => {
    ...
    const index = bodyIndex(built({ turret: { element: { mesh } }, rock: { node: plain } }))

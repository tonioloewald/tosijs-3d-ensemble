...
const garden: Ensemble = {
  ...
  pieces: [
    { id: 'glasshouse', mesh: 'building', at: [0, 0, 0], values: { built: 1898 } },
    {
      id: 'oak',
      mesh: 'tree',
      at: [12, 0, 4],
      subsystems: [{ match: 'Branch$', label: 'limb', features: { pruneable: {} } }],
    },
    { id: 'pump', mesh: 'pillar', at: [-6, 0, 2], features: { irrigation: { litresPerHour: 400 } } },
  ],
  links: [{ from: 'pump', to: 'oak', kind: 'irrigates', values: { delay: 0.4 } }],
  points: [{ id: 'gate', at: [0, 0, 20], kind: 'entrance' }],
  zones: [
    { id: 'quiet', at: [0, 0, 0], radius: 30, kind: 'quiet-area', values: { decibels: 45 } },
    { id: 'wifi', at: [4, 2, 0], radius: 25, kind: 'wifi', values: { ssid: 'GARDEN' } },
  ],
}
...
describe('the format has no domain', () => {
  ...
  it('accepts any zone or point kind, because kinds are conventions', () => {
    ...
    const problems = validate(
      { ...garden, zones: [{ id: 'z', at: [0, 0, 0], radius: 1, kind: 'anything-at-all' }] },
      { checkRegistry: false }
    )
  ...
  it('lets a part of a composite mesh carry FEATURES, not hit points', () => {
    ...
    expect(garden.pieces[1]?.subsystems?.[0]?.features).toEqual({ pruneable: {} })
  })

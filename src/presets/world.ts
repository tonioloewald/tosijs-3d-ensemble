  ...
  registerFeature<InteractiveHandle>({
    ...
    bind(_piece, cfg, ctx) {
      ...
      everyFrame(ctx, () => {
        ...
        near = viewer
          ? distanceTo(viewer, ctx.at) <= (reach > 0 ? reach : Infinity)
          : false;
      });
  ...
  registerFeature<AnimationHandle>({
    ...
    schema: {
      ...
      properties: {
        clip: {
          ...
          title: "Clip",
          description:
            "name of the animation in the model; blank plays the first",
        },
    ...
    */
    bind(_piece, cfg, ctx) {
      ...
      const groups = (): Clip[] => {
        const node =
          (ctx.element as unknown as { mesh?: AnimatedNode } | null)?.mesh ??
  ...
  registerFeature({
    ...
    */
    link(handle, ctx) {
      ...
      const interactive = ctx.feature("interactive") as
        | InteractiveHandle
        | undefined;
  ...
  registerFeature({
    ...
    bind(_piece, cfg, ctx) {
      ...
      everyFrame(ctx, (_dt, now) => {
        ...
        rot[axis === "x" ? 0 : axis === "y" ? 1 : 2] = spinAngle(
          rate,
          now - start
        );
        writeBody(body, { rot });

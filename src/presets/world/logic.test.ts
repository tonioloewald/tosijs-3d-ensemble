...
describe('doors', () => {
  ...
  it('reopens from a partial close without jumping', () => {
    const closing = stepDoor(run(closedDoor(), 8, 0.1, true), 0.2, { seconds: 1 }, false)
  ...
  it('stays open indefinitely when autoClose is 0 and something still wants it', () => {
    let s = run(closedDoor(), 12, 0.1, true)
    for (let i = 0; i < 50; i++) s = stepDoor(s, 1, { seconds: 1, autoClose: 0 }, true)
    expect(s.phase).toBe('open')
  })
  ...
  it('eases rather than travelling linearly', () => {
    ...
    expect(doorAmount({ phase: 'opening', progress: 1, elapsed: 0 }, 90)).toBe(90)
  })
})

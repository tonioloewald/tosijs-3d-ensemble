...
export const closedDoor = (): DoorState => ({ phase: 'closed', progress: 0, elapsed: 0 })
 ...
 */
export function stepDoor(state: DoorState, dt: number, rules: DoorRules, wants: boolean): DoorState {
  const seconds = Math.max(0.0001, rules.seconds)
  const step = dt / seconds

  // Shut and wanted shut: nothing to do but age.
  if (state.phase === 'closed' && !wants) return { ...state, elapsed: state.elapsed + dt }

  if (state.phase === 'open') {
    if (wants) return { phase: 'open', progress: 1, elapsed: 0 }
    const hold = rules.autoClose ?? 0
    const elapsed = state.elapsed + dt
    if (hold > 0 && elapsed < hold) return { phase: 'open', progress: 1, elapsed }
  ...
  */
  ...
  if (!wants && progress <= 0) return { phase: 'closed', progress: 0, elapsed: 0 }
  return {
...
export const doorAmount = (state: DoorState, full: number): number => ease(state.progress) * full
 ...
 */
export function flicker(base: number, amount: number, time: number, seed = 1): number {
  if (amount <= 0) return base
  // Two incommensurable sine terms: cheap, and it does not repeat on a beat the
  // eye can pick out the way a single sine does.
  const n = Math.sin(time * 11.3 + seed * 7.7) * 0.6 + Math.sin(time * 23.1 + seed * 3.1) * 0.4

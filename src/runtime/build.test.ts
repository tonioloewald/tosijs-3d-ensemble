  ...
  pieces: [
    {
      ...
      mesh: "R",
      at: [0, 0, 0],
      features: { destroyable: { hp: 16 } },
    },
    {
      id: "field",
      mesh: "F",
      at: [0, 10, 0],
      features: { protector: { source: "reactor" } },
    },
  ],
...
describe("buildEnsemble", () => {
  it("binds every piece before any link runs", () => {
    ...
    expect(order.slice(0, firstLink).every((s) => s.startsWith("bind:"))).toBe(
      true
    );
    expect(order.slice(firstLink).every((s) => s.startsWith("link:"))).toBe(
      true
    );
  });
  ...
  it("resolves a neighbour in link regardless of file order", () => {
    ...
    const reversed: Ensemble = {
      ...two,
      pieces: [two.pieces[1]!, two.pieces[0]!],
    };
    ...
    buildEnsemble(
      {
        name: "s",
        scale: 2,
        pieces: [{ id: "a", mesh: "X", at: [1, 2, 3], scale: 3 }],
      },
      {
        scene: fakeScene(),
        origin: [100, 0, 0],
    ...
    buildEnsemble(
      {
        name: "s",
        scale: 2,
        pieces: [{ id: "a", mesh: "X", at: [0, 0, 0], scale: [1, 3, 5] }],
      },
      {
        scene: fakeScene(),
  ...
  it("returns validation problems without refusing to build", () => {
    const built = buildEnsemble(
      { name: "", pieces: [{ id: "a", mesh: "X", at: [0, 0, 0] }] },
      {
        scene: fakeScene(),
      }
    );
